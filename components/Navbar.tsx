    ...
    <>
      <nav 
        className={`fixed top-4 md:top-6 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-700 ease-in-out
          ${scrolled ? 'w-[95%] max-w-[1400px] py-2' : 'w-[98%] max-w-[1700px] py-4'}
        `}
      >
        <div className={`
        ...
        `}>
          ...
          {/* Logo Section */}
          <div 
            className="flex items-center gap-3 lg:gap-4 cursor-pointer group flex-shrink-0"
          ...
          >
            <div className={`
                ...
                ${scrolled ? 'w-10 h-10 lg:w-12 lg:h-12' : 'w-12 h-12 lg:w-16 lg:h-16'}
            `}>
              <img 
                src={LOGO_URL} 
                alt="MF" 
            ...
            <span className="font-sans font-black tracking-tighter text-brand-dark dark:text-white text-base lg:text-lg xl:text-xl hidden sm:block uppercase">
              MF <span className="text-brand-cyan">PROD</span>
            </span>
          </div>

          {/* Desktop Navigation */}
          <div className="hidden lg:flex items-center gap-4 xl:gap-8 flex-grow justify-center">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  <button
                    ...
                    className={`
                      uppercase tracking-[0.15em] text-[11px] xl:text-[13px] font-black transition-all duration-300 relative py-4 flex items-center gap-2
                  ...
                  {isPrestations && (
                    ...
                    `}>
                      <div className="bg-white/98 dark:bg-brand-dark-soft/98 backdrop-blur-2xl border border-black/5 dark:border-white/10 rounded-[2rem] p-6 w-[320px] shadow-2xl">
                        <div className="flex flex-col gap-1">
                          {prestationsLinks.map((link) => (
                            <button
                              key={link.label}
                              onClick={() => handlePrestationClick(link.view)}
                              className="text-left px-4 py-2.5 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 text-[10px] uppercase tracking-[0.2em] font-black text-brand-dark/70 dark:text-white/70 hover:text-brand-magenta transition-all"
                            >
                              {link.label}
                            </button>
                          ))}
                ...
                return (
                  <div key={item.label} className="w-full flex flex-col items-center">
                    <button
                      ...
                      className={`text-4xl sm:text-5xl font-sans font-black uppercase tracking-tighter transition-colors ${currentView === item.view ? 'text-brand-cyan' : 'text-brand-dark dark:text-white'}`}
                    >
                      {item.label}
                    </button>
                    
                    {isPrestations && mobilePrestationsOpen && (
                      <div className="flex flex-col items-center gap-5 mt-6 animate-fade-in-up bg-black/5 dark:bg-white/5 rounded-[2.5rem] p-8 w-full max-w-sm">
                        {prestationsLinks.map((link) => (
                          <button
                            key={link.label}
                            onClick={() => handlePrestationClick(link.view)}
                            className="text-base font-black text-brand-dark/60 dark:text-white/60 hover:text-brand-magenta uppercase tracking-widest text-center"
                          >
                            {link.label}
                          </button>
                        ))}
